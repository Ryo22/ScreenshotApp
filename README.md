...
※自動モードで撮影を開始すると、現在の操作状況と停止ショートカットを案内するオーバーレイが画面手前に表示されます。
...
※デフォルトで有効になっており、初期設定として一般的なビューア向けの座標 (X: 29, Y: 494) がセットされています。

1. 必要に応じて**「🖱 画面位置を取得」**ボタンを押し、画面上の「自動クリックしたい場所（例：次へボタン）」をクリックして座標を取得・変更する。
2. **タップ待機時間（秒）**: クリックした後に次の処理に移るまでの待機時間を設定する（デフォ: 0.1秒）。
3. 「▶ 開始」を押すと、`クリック → 保存 → 待機 → クリック ...` のループが実行されます。
4. **【便利機能】自動撮影の自動停止**: ページめくりを行っても画像の変化がなくなった場合（最終ページ等）、撮影を自動で終了します。
5. **【便利機能】重複画像の自動削除**: 撮影停止時に、最後に連続して保存された2枚の画像が同一だった場合、無駄な重複分として最後の一枚を自動で削除します。
